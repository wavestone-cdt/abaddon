...
def getInitialBeaconLine(l1):
  ...
  q2['query']['query_string']['query'] = "beacon_id:\"%s\" AND cslogtype:beacon_newbeacon AND agent.name:%s"%(l1['_source']['beacon_id'],l1["_source"]['agent']['name'])
...
def enrichAllLinesWithBeacon(l1,b):
  ...
  q3['query']['query_string']['query'] = "beacon_id:\"%s\" AND agent.name:%s AND NOT tags:enriched_v01"%(l1['_source']['beacon_id'],l1["_source"]['agent']['name'])
  r3 = es.search(index="rtops-*", body=q3)
  print(len(r3['hits']['hits']))
  for l1 in r3['hits']['hits']:
    ...
    for field in ["target_hostname","target_ipext","target_os","target_osversion","target_pid","target_user"]:
      try:
        ...
        pass
    es.update(l1['_index'],l1['_id'],doc_type=l1['_type'],body={'doc':l1['_source']})
    tagsSet = tagsSet + 1
    #sys.stdout.write('.')
    #sys.stdout.flush()
  return(tagsSet)
...
def setTags(tag,lst):
  for l in lst:
    ...
    r = es.update(l['_index'],l['_id'],doc_type=l['_type'],body={'doc':l['_source']})
    #sys.stdout.write('.')
    #sys.stdout.flush()
